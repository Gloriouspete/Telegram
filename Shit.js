...
app.get('/', (req, res) => {
    res.send('Server is working correctly no fpit');
});
...
app.get('/api/v1/version', (req, res) => {
    const userid = req.user.userid;
    const versioncontrol = '26'
    res.send(versioncontrol)
})

app.get('/api/v1/notification', (req, res) => {
    const userid = req.user.userid;
    const message = 'Dear esteemed customers, Corporate Gifting is currently down globally, Kindly use other types of data for the time being. The technical team is currently working on the issue and it would be resolved soon.   Use our website ( heydata.com.ng ) for transactions for now'
    res.send(message)
})
